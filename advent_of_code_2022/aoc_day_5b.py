...
def aoc_day_5b(input_text_file):
    containers = []
    instructions = []
    ...
    for instruction in parsed_instructions:
        ...
        items = []
        for i in range(num_containers_to_be_moved):
            items.append(transformed_containers[container_from][i])

        items.reverse()

        for i in range(len(items)):
            transformed_containers[container_to].insert(0, items[i])
