...
def extract_maps(input_text):
    ...
    lowest_location = 1000000000000000000000000000000000

    for i in range(len(seeds_final)):
        print('i: ', i)
        single_seed_range = seeds_final[i]
        seed_df_list = []
        ...
        min_val = seeds_df.agg(_min('seed')).collect()[0][0]
        max_val = seeds_df.agg(_max('seed')).collect()[0][0]
        seed_to_soil_df = seed_to_soil_df.filter(col('seed').between(min_val, max_val))
        ...
        min_val = seeds_df.agg(_min('soil')).collect()[0][0]
        max_val = seeds_df.agg(_max('soil')).collect()[0][0]
        soil_to_fertilizer_df = soil_to_fertilizer_df.filter(col('soil').between(min_val, max_val))
        ...
        min_val = seeds_df.agg(_min('fertilizer')).collect()[0][0]
        max_val = seeds_df.agg(_max('fertilizer')).collect()[0][0]
        fertilizer_to_water_df = fertilizer_to_water_df.filter(col('fertilizer').between(min_val, max_val))
        ...
        min_val = seeds_df.agg(_min('water')).collect()[0][0]
        max_val = seeds_df.agg(_max('water')).collect()[0][0]
        water_to_light_df = water_to_light_df.filter(col('water').between(min_val, max_val))
        ...
        min_val = seeds_df.agg(_min('light')).collect()[0][0]
        max_val = seeds_df.agg(_max('light')).collect()[0][0]
        light_to_temperature_df = light_to_temperature_df.filter(col('light').between(min_val, max_val))
        ...
        min_val = seeds_df.agg(_min('temperature')).collect()[0][0]
        max_val = seeds_df.agg(_max('temperature')).collect()[0][0]
        temperature_to_humidity_df = temperature_to_humidity_df.filter(col('temperature').between(min_val, max_val))
        ...
        min_val = seeds_df.agg(_min('humidity')).collect()[0][0]
        max_val = seeds_df.agg(_max('humidity')).collect()[0][0]
        humidity_to_location_df = humidity_to_location_df.filter(col('humidity').between(min_val, max_val))
        ...
        location_array = seeds_df.toPandas().to_numpy()
        current_location = np.min(location_array)
