...
def ranges(input_range_list, max_range_size):
    result_list = []
    for input_range in input_range_list:
        if len(input_range) > max_range_size:
            steps = len(input_range) // max_range_size
            for step in range(steps):
                partial_range = range(input_range.start + (step)*max_range_size, input_range.start + (step)*max_range_size + max_range_size)
                result_list.append(partial_range)
        else:
            result_list.append(input_range)
    return result_list
...
def extract_maps(input_text):
    ...
    for i in range(int(len(seeds) / 2)):
        seeds_final.append(range(seeds[i * 2], seeds[i * 2] + seeds[i*2 + 1]))

    seeds_final = ranges(seeds_final, 3000000)
    print('seeds_final after split: ', seeds_final)
    print('len of new ranges: ', len(seeds_final))
    ...
    for i in range(len(seeds_final)):
        print('processing seed range: ', i)

        for seed in seeds_final[i]:
            soil = None
            fertilizer = None
            temperature = None
            water = None
